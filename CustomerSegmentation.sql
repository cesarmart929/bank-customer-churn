...
-- Why are Spanish under-30's churning at double the rate of French under-30's even with nearly identical average account balances of €59,000?

SELECT
  100 * AVG (CASE WHEN Geography = "Germany" AND Age <= 30 THEN Exited ELSE NULL END) AS a,
  100 * AVG (CASE WHEN Geography = "France" AND Age <= 30 THEN Exited ELSE NULL END) AS b,
  100 * AVG (CASE WHEN Geography = "Spain" AND Age <= 30 THEN Exited ELSE NULL END) AS c
FROM
  `coherent-window-466113-k2.bank_churn.customers`;